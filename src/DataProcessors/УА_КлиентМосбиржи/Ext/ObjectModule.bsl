...
Функция ДанныеИнструментов(ТикерыМассив, КолонкиМассив = Неопределено, РежимТоргов = "TQBR") Экспорт
	
	ТикерыСтрокой = СтрСоединить(ТикерыМассив, ",");
	
	Если Не ЗначениеЗаполнено(КолонкиМассив) Тогда
		КолонкиМассив = _КолонкиИнструментовПоУмолчанию();
	КонецЕсли;
	
	КолонкиСтрокой  = СтрСоединить(КолонкиМассив, ",");
	
	ПараметрыЗапроса = _ДанныеИнструментовОписаниеПараметров();
	ПараметрыЗапроса.Вставить("securities.columns", КолонкиСтрокой);
...
Функция _ДанныеИнструментовОписаниеПараметров()
	...
	Параметры.Вставить("securities.columns");
	Параметры.Вставить("iss.meta",   "off");
	Параметры.Вставить("iss.data",   "on");
	Параметры.Вставить("iss.only",   "securities");

	Возврат Параметры;
	
КонецФункции

Функция _КолонкиИнструментовПоУмолчанию()

	Колонки = Новый Массив;
	
	Колонки.Добавить("SECID");
	Колонки.Добавить("SHORTNAME");
	Колонки.Добавить("LOTSIZE");
	Колонки.Добавить("PREVPRICE");
